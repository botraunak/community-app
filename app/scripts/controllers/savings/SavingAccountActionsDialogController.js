(function (module) {
    mifosX.controllers = _.extend(module, {
        SavingAccountActionsDialogController: function (scope, resourceFactory, location, routeParams, dateFilter, accountData, $uibModalInstance) {

            scope.action = accountData.action || "";
            scope.accountId = accountData.id;
            scope.savingAccountId = accountData.id;
            scope.formData = {};
            ...
            switch (scope.action) {
                ...
                case "applyAnnualFees":
                    resourceFactory.savingsResource.get({accountId: accountData.id, resourceType: 'charges', chargeId: routeParams.chargeId},
                        function (data) {
                ...
                case "close":
                    ...
                    resourceFactory.savingsResource.get({accountId: accountData.id, fields:'summary'}, function (accountData) {
            ...
            scope.submit = function () {
                ...
                if (scope.action == "deposit" || scope.action == "withdrawal" || scope.action == "modifytransaction" || scope.action=="postInterestAsOn") {
                    ...
                    resourceFactory.savingsResource.update({accountId: accountData.id, resourceType: 'charges', chargeId: routeParams.chargeId}, this.formData,
                        function (data) {
                            $uibModalInstance.dismiss();
                        });
                } else if (scope.action == "deletesavingcharge") {
                    resourceFactory.savingsResource.delete({accountId: accountData.id, resourceType: 'charges', chargeId: routeParams.chargeId}, this.formData,
                        function (data) {
                            $uibModalInstance.dismiss();
                        });
                } else if (scope.action == "paycharge" || scope.action == "waive" || scope.action == "inactivate") {
                    params = {accountId: accountData.id, resourceType: 'charges', chargeId: routeParams.chargeId, command: scope.action};
                    ...
                    if (scope.action == "approve") {
                        ...
                        params = {accountId: accountData.id, resourceType: 'charges', chargeId: routeParams.chargeId, command: 'paycharge'};
                        if (this.formData.dueDate) {
                            this.formData.dueDate = dateFilter(this.formData.dueDate, scope.df);
                        }
    ...
    mifosX.ng.application.controller('SavingAccountActionsDialogController', ['$scope', 'ResourceFactory', '$location', '$routeParams', 'dateFilter', 'accountData', '$uibModalInstance', mifosX.controllers.SavingAccountActionsDialogController]).run(function ($log) {
        $log.info("SavingAccountActionsDialogController initialized");
    });
}(mifosX.controllers || {}));
